...
> **Note on `tooling/exec-diagnostic-onepager/`** — the scripts in `tooling/exec-diagnostic-onepager/` need a real browser (playwright) or
...
> lacks. They are author-side proofs, so they live in `tooling/exec-diagnostic-onepager/` and are run locally:
...
> live at `tooling/exec-diagnostic-onepager/` in the repository root — *outside* the `skills/`
...
> install chromium`, then `python tooling/exec-diagnostic-onepager/<script>.py`. Everything under
