...
**Commercial angle.** The demo IBM's own finance can use ([INTERNAL], per the kickoff), and the objection-handler for every client who says "our data is too sensitive for AI" — because the governance pattern *is* the deliverable. Feeds requirements to the Ask Finance product team.
...
1. Day 1–2: assemble the [INTERNAL] quarter pack (sanitized extracts agreed with data owner).
...
### Risks & mitigations
- **Data sensitivity (the big one):** sanitized [INTERNAL] extracts only during the challenge; DLP scrub before upload; tenant isolation; per-role assistants; no production credentials.
